...
import { computeProjectTotals } from "../core/calc";
...
export default function ProjectsScreen({ state, setState, goSettings, openProject }) {
  ...
  // ✅ Artık teklif ekranındaki gibi hesap
  function projectGrandTotal(p) {
    return computeProjectTotals(state, p).grandTotal || 0;
  }

  // ✅ Toplam Ciro (listede görünen toplamlar)
  const totalCiro = useMemo(() => {
    return projectsSorted.reduce((sum, p) => sum + projectGrandTotal(p), 0);
  }, [projectsSorted, state.settings]);
  ...
  function createProject() {
    ...
    const p = {
      ...
      offerDateISO: now,
      currentVersion: "A",
      // teklif durumu sende vardı — yoksa bile sorun değil (sonra geri ekleriz)
      status: "Teklif",
      items: [],
      accessories: [],
    };
    ...
    <div style={page}>
      <div style={wrap}>
        ...
        <div style={{ ...card, marginTop: 12, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            <div style={{ fontWeight: 950 }}>Toplam Ciro</div>
            <div style={{ fontSize: 12, color: "#666", fontWeight: 800 }}>Listede görünen tüm tekliflerin toplamı</div>
          </div>
          <div style={{ fontWeight: 950, fontSize: 18 }}>{currency(totalCiro)}</div>
        </div>
              ...
              <div key={p.id} style={card}>
                <div style={{ display: "flex", justifyContent: "space-between", gap: 10 }}>
                  ...
                  <div style={{ textAlign: "right" }}>
                    <div style={{ fontWeight: 950 }}>{currency(projectGrandTotal(p))}</div>
